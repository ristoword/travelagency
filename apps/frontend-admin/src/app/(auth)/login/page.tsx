...
import { Loader2, Eye, EyeOff, Globe, TrendingUp, Users, FileText, Plane, Star } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const [tenantSlug, setTenantSlug] = useState('demo-agenzia');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!tenantSlug.trim()) { setError('Inserisci il codice agenzia'); return; }
    if (!email.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) { setError('Email non valida'); return; }
    if (password.length < 4) { setError('Password troppo corta'); return; }
